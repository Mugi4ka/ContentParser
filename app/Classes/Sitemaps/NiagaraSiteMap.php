...
class NiagaraSiteMap implements \App\Interfaces\SiteMapInterface
{

    public function createSiteMap($siteLink)
    {
        $path = storage_path('app/niagara.xml');
        SitemapGenerator::create($siteLink)
            ->hasCrawled(function (Url $url) {
                if ($url->segment(1) == 'dushevye-kabiny') {
                    $url->setPriority(1.0);
                    return $url;
                } else {
                    return "";
    ...
    {
        $itemLinks = [];
        $xml = XmlParser::load(storage_path('app/niagara.xml'));
